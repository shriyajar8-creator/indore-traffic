...
export const CivilianApp: React.FC<CivilianAppProps> = ({
   ...
   * These values are populated directly from the Admin -> Backend -> Socket.IO event.
   */
  const [alertRoadName, setAlertRoadName] = useState<string | null>(null);
  const [alertReason, setAlertReason] = useState<string | null>(null);
  const [alertTitle, setAlertTitle] = useState('ROAD CLOSURE ALERT');
  ...
  /*
   * Keep the latest Auto Reroute setting available to the Socket.IO listener
   * without recreating the Socket.IO listeners every time the checkbox changes.
   */
  const autoRerouteRef = useRef(autoRerouteEnabled);

  /*
   * Prevent the same Admin action from showing twice because the backend
   * sends both:
   *
   * road.closed
   * alert.broadcast
   */
  const lastRealtimeAlertRef = useRef<string | null>(null);

  useEffect(() => {
    autoRerouteRef.current = autoRerouteEnabled;
  }, [autoRerouteEnabled]);
  ...
  const fetchDirectionsRoute = async (
    ...
    try {
      ...
      setDirectionsError(
        err.message || 'Failed to query directions engine'
      );
    ...
    try {
      ...
      if (notifRes.ok) {
        ...
        if (notifData.notifications) {
          setLiveNotifications(notifData.notifications);
        }
      }
    } catch (err) {
  ...
  useEffect(() => {
     ...
     * This 5-second polling remains only as a backup synchronization mechanism
     * for other live data.
     */
  ...
  useEffect(() => {
    ...
    const showRealtimeAlert = ({
      ...
      /*
       * Ignore duplicate delivery of the same Admin action.
       */
      if (lastRealtimeAlertRef.current === alertKey) {
        return;
      }

      lastRealtimeAlertRef.current = alertKey;
      ...
      setAlertRoadName(roadName || null);
      setAlertReason(reason || null);

      setToastBanner(
        roadName
          ? `${title}: ${roadName}${reason ? ` — ${reason}` : ''}`
          : message
      );
    ...
    /*
     * Initial synchronization when the User device connects.
     */
     ...
     *
     * This is sent by the backend after Admin creates/updates an
     * incident or closes a road.
     */
    socket.on(
      'alert.broadcast',
      (data: { notification: SystemNotification }) => {
        ...
        const roadName =
          notification.affectedRoadId || 'Affected corridor';

        const alertKey =
          notification.id ||
          `${notification.title}-${notification.message}-${notification.affectedRoadId || ''}`;

        /*
         * If road.closed already displayed this exact notification,
        ...
        showRealtimeAlert({
          title: notification.title || 'TRAFFIC ALERT',
          message:
     ...
     * ============================================================
     *
     * This is the most important event for your two-device demo.
     *
     * Admin clicks:
     *       CLOSE ROAD
     *
     ...
     *       road.closed
     *
     * User device receives it immediately.
     */
    socket.on('road.closed', (data: any) => {
      const roadId = data?.roadId || data?.road?.id || '';

      const roadName =
        data?.road?.name ||
        data?.roadName ||
        data?.notification?.affectedRoadId ||
        roadId ||
        'Road';
      ...
      /*
       * Prefer the notification ID because the backend sends
       * the same notification again through alert.broadcast.
       */
      const alertKey =
        notification?.id ||
      ...
      /*
       * Update the road state immediately from the Socket.IO event.
       */
      if (data?.road) {
        setLiveRoads((currentRoads) => {
          const exists = currentRoads.some(
            (road) => road.id === data.road.id
          );
          ...
          return currentRoads.map((road) =>
            road.id === data.road.id
              ? { ...road, ...data.road, isClosed: true }
              : road
          );
        });
      }
      ...
      /*
       * Refresh the complete live state after displaying the
       * immediate Socket.IO alert.
       */
      refreshLiveData();
    });
    ...
    socket.on('road.opened', (data: any) => {
      ...
      /*
       * If the reopened road was the active alert road,
       * clear the active alert state.
       */
      setActiveAlert(null);
    ...
    /*
     * New incident created by Admin / authority.
     */
    socket.on('incident.created', (data: any) => {
      ...
      /*
       * If this incident already came with an alert notification,
       * alert.broadcast will handle the main popup.
       */
      ...
      const title =
        inc.title || 'Live Incident Reported';
      ...
      const alertKey =
        `incident-${inc.id || `${title}-${roadName}`}`;
      ...
      showRealtimeAlert({
        title:
          ...
          inc.severity === 'HIGH'
            ? 'CRITICAL INCIDENT'
            : 'LIVE INCIDENT',
        message: `${message}${locStr}`,
        roadName,
        reason: inc.description || undefined,
        alertKey,
        shouldReroute:
          inc.severity === 'CRITICAL' ||
          inc.severity === 'HIGH'
      ...
      if (inc.status === 'RESOLVED') {
        setToastBanner(
          `RESOLVED: ${
            ...
            inc.roadName || 'corridor'
          } cleared by authorities`
        );
      } else {
        setToastBanner(
          `UPDATED INCIDENT: ${
            inc.title || 'Incident update'
          } on ${
            inc.roadName || 'corridor'
          }`
        );
      ...
      {toastBanner && (
        <div className="bg-gradient-to-r from-red-600 via-amber-600 to-red-600 text-white text-xs font-bold px-4 py-2 flex items-center justify-between shadow-xl z-40 animate-pulse">
          ...
          <button
            ...
            className="text-white hover:text-slate-200 text-xs font-black"
          >
            ✕
          </button>
        </div>
            ...
            <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3 shadow-lg">
              ...
              {liveIncidents.some(
                ...
                <div className="bg-amber-500/10 border border-amber-500/40 p-2.5 rounded-lg flex items-center space-x-2 text-xs text-amber-400">
                  ...
                  <span>
                    Active obstacle / construction zone reported
                    on corridor.
                  </span>
                </div>
              )}
              ...
              {!routeSwitched ? (
                ...
                <div className="bg-emerald-500/10 border border-emerald-500/30 p-2.5 rounded-lg flex items-center space-x-2 text-xs text-emerald-400">
                  ...
                  <span>
                    Switched to optimal alternate route via
                    Eastern Bypass.
                  </span>
                </div>
              )}
            ...
            <div className="space-y-2">
              ...
              liveIncidents.length === 0 ? (
                <div className="bg-slate-950 border border-slate-800 p-3 rounded-xl text-center text-xs text-slate-500">
                  No active incidents or advisories. Corridors
                  clear.
                </div>
              ) : (
            ...
            {reportSubmittedSuccess ? (
              <div className="py-6 text-center space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Traffic police officers will inspect and update
                  the live network.
                </p>
              </div>
            ) : (
